...
describe('home', () => {
  ...
  it('leads with the net position and shows the testnet chip only on testnet', () => {
    const html = renderHome(vm, names, t, null, { showNetworkChip: true, canSettle: true });
    expect(html).toContain('1,200');
    expect(html).toContain('You are owed');
    expect(html).toContain('Testnet');
    expect(renderHome(vm, names, t, null, { showNetworkChip: false, canSettle: true })).not.toContain('Testnet');
  });

  it('shows a degraded banner above the ledger without hiding it', () => {
    const html = renderHome(vm, names, t, { kind: 'offline', source: 'cache' }, { showNetworkChip: true, canSettle: true });
